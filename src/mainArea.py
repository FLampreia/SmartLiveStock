...
while True:
    ...
    # Fora do loop, desenhar a área no frame para visualização
    ...
    if result[0] is not None and result[0].boxes.id is not None:
        ...
        for box, track_id, class_id in zip(xyxy, ids, class_ids):
            ...
            # Verificar se centro está dentro da ROI
            inside = cv2.pointPolygonTest(roi_polygon, (cx, cy), False)

            if inside >= 0 and track_id not in unique_ids:
                unique_ids.add(track_id)
                sheep_count += 1
                print(f"({sheep_count}) Ovelha entrou na área: {track_id}")
